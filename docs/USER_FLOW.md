...
| 引用反馈 | 采用、不采用、不相关、自动入队、反馈下载 | 让 RAG 引用从黑盒展示变成可干预数据 |
| 改稿对比 | 初稿问题、一审建议、终稿、提升维度 | 让双检双审过程可解释 |
| PM 后台诊断 | 低分维度、风险点、资料缺口、内部迭代动作 | 把评分变成内容质量管理入口 |

## 3. 输出区为什么用四个 Tab

- 生成内容：用户第一关注点，默认展示。
- 引用案例：展示知识库依据，并允许用户反馈采用、不采用或不相关；反馈自动进入队列。
- 审核维度：展示固定质量标准，体现可控性。
- 综合评分：面向用户展示是否可发布、是否需要人工复核；低分诊断默认进入 PM 后台。
...
终审评分低于或等于 7 分的维度不会直接展示给普通用户，而是进入 PM 后台诊断：
...
| 内部沉淀什么 | 变成 Prompt、知识库或审核规则的迭代方向 |
